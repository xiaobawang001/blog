...
import { patchSidebar } from './sidebar-routes'

const GITHUB_USER = 'xiaobawang001'
const SITE_REPO = 'xiaobawang001.github.io'
...
const vitePressConfig = defineConfig({
  ...
  themeConfig: {
    ...
    editLink: {
      pattern: `https://github.com/${GITHUB_USER}/${SITE_REPO}/edit/master/docs/:path`,
      text: '在 GitHub 上编辑',
    },
    outline: {
      label: '文章目录',
...
const finalConfig = withSidebar(vitePressConfig, {
  ...
  excludeByGlobPattern: ['使用指南/发布新文章.md'],
})

// 用集中配置覆盖侧边栏标题与排序，不再需要子目录的 index.md
if (finalConfig.themeConfig?.sidebar) {
  for (const items of Object.values(finalConfig.themeConfig.sidebar)) {
